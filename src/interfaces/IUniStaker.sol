...
interface IUniStaker {
  ...
  event StakeDeposited(
    address owner,
    UniStaker.DepositIdentifier indexed depositId,
    uint256 amount,
  ...
  event StakeWithdrawn(
    UniStaker.DepositIdentifier indexed depositId, uint256 amount, uint256 depositBalance
  );

  /// @notice Emitted when a deposit's delegatee is changed.
  event DelegateeAltered(
    UniStaker.DepositIdentifier indexed depositId, address oldDelegatee, address newDelegatee
  );
  ...
  ///
  /// Note that the contract tracks the unclaimed rewards internally with the scale factor included,
  /// in order to avoid the accrual of precision losses as users takes actions that cause rewards to
  /// be checkpointed. This external helper method is useful for integrations, and returns the value
  /// after it has been scaled down to the reward token's raw decimal amount.
  ...
  function stake(uint96 _amount, address _delegatee) external returns (UniStaker.DepositIdentifier);
  ...
  function permitAndStake(
    ...
    bytes32 _r,
    bytes32 _s
  ) external returns (UniStaker.DepositIdentifier _depositId);

  /// @notice Stake tokens to a new deposit on behalf of a user, using a signature to validate the
  /// user's intent. The caller must pre-approve the staking contract to spend at least the would-be
  /// staked amount of the token.
  /// @param _amount Quantity of the staking token to stake.
  /// @param _delegatee Address to assign the governance voting weight of the staked tokens.
  /// @param _beneficiary Address that will accrue rewards for this stake.
  ...
  function stakeMore(UniStaker.DepositIdentifier _depositId, uint96 _amount) external;
  ...
  /// the deposit.
  function alterDelegatee(UniStaker.DepositIdentifier _depositId, address _newDelegatee) external;
  ...
  /// the deposit.
  function alterBeneficiary(UniStaker.DepositIdentifier _depositId, address _newBeneficiary)
    external;

  /// @notice For an existing deposit, change the beneficiary to which staking rewards are accruing
  /// on behalf of a user, using a signature to validate the user's intent.
  /// @param _depositId Unique identifier of the deposit which will have its beneficiary altered.
  /// @param _newBeneficiary Address of the new rewards beneficiary.
  /// @param _depositor Address of the user on whose behalf this stake is being made.
  /// @param _deadline The timestamp after which the signature should expire.
  ...
  /// sender's account.
  function withdraw(UniStaker.DepositIdentifier _depositId, uint96 _amount) external;

  /// @notice Withdraw staked tokens from an existing deposit on behalf of a user, using a signature
  /// to validate the user's intent.
  /// @param _depositId Unique identifier of the deposit from which stake will be withdrawn.
  /// @param _amount Quantity of staked token to withdraw.
  /// @param _depositor Address of the user on whose behalf this stake is being made.
  /// @param _deadline The timestamp after which the signature should expire.
  ...
  /// @dev returns the unpacked Deposit struct: balance, owner, delegatee, beneficiary
  function deposits(UniStaker.DepositIdentifier)
    external
    view
    returns (uint96, address, address, address);
  ...
  /// in scaled reward tokens per second, using the SCALE_FACTOR.
  ...
  /// calculated and stored. The difference between the global value and this value can be used to
  /// calculate the interim rewards earned by given account.
  ...
  function scaledUnclaimedRewardCheckpoint(address) external view returns (uint256);
  ...
  /// @notice The next unused nonce for an address
